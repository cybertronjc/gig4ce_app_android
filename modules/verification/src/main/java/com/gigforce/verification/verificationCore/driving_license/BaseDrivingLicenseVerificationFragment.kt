package com.gigforce.verification.verificationCore.driving_license
...
import com.gigforce.verification.mainverification.drivinglicense.DrivingLicenseViewModel
...
@AndroidEntryPoint
abstract class BaseDrivingLicenseVerificationFragment constructor(
    fragmentName: String
) : BaseFragment2<DrivingLicenseFragmentBinding>(
    fragmentName = fragmentName,
    layoutId = R.layout.driving_license_fragment,
    ...
    override fun viewCreated(
        viewBinding: DrivingLicenseFragmentBinding,
    ...
    private fun checkForCroppedImage() {
        ...
        if (navFragmentsData.getData() != null) {
            if (navFragmentsData.getData().getString(StringConstants.IMAGE_CROP_URI.value)
                    ?.isNotEmpty() == true
            ) {
    ...
    private fun observer() {
        ...
        viewModel.getVerifiedStatus(getUserId())
...
//                Data("fathername", viewBinding.fatherNameTil.editText?.text.toString()),
        ...
        activeLoader(true)
        viewModel.getKycVerificationResult("DL", list, getUserId())
    }
    ...
    private fun getImageDimensions(uri: Uri): Size {
        ...
        BitmapFactory.decodeFile(uri.toFile().absolutePath, options)
...
//        options.setMaxBitmapSize(1000)
    ...
    abstract fun shouldEnableOcr(): Boolean

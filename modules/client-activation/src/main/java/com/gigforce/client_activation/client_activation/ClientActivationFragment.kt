...
import com.gigforce.client_activation.client_activation.models.LessonModel
import com.gigforce.client_activation.client_activation.models.Media
import com.gigforce.core.core.base.BaseFragment
import com.gigforce.core.core.genericadapter.PFRecyclerViewAdapter
import com.gigforce.core.core.genericadapter.RecyclerGenericAdapter
            ...
            run {
                if (FirebaseAuth.getInstance().currentUser?.uid == null) {
                    ...
                    tv_mark_as_interest_role_details.setOnClickListener {
                        ...
                        navigationDire.navigateTo("client_activation/login")
//                        navigate(R.id.Login)
                    }
                } else {
                    tv_mark_as_interest_role_details.setOnClickListener {
    ...
    fun markAsInterestClick(jpApplication: JpApplication?) {
        if (jpApplication == null || jpApplication.status == "" || jpApplication.status == "Draft") {
            if (mClientViaDeeplink == true) {
                ...
                navigationDire.navigateTo(
                    "client_activation/applicationScreen", bundleOf(
                        StringConstants.JOB_PROFILE_ID.value to viewModel.observableJobProfile.value?.profileId
                    )
                )
//                navigate(

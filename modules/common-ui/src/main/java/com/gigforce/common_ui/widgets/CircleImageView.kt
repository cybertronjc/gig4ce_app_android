...
open class CircleImageView : AppCompatImageView {
    ...
    private fun init() {
        ...
        if (mDashedBorder) {
            val dashPath =
                DashPathEffect(floatArrayOf(20f, 15f), 1.0.toFloat())
            mBorderPaint.pathEffect = dashPath
            mBorderPaint.style = Paint.Style.STROKE
        }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (mBitmap != null) {
            ...
            canvas.drawCircle(
                mDrawableRect.centerX(),
                mDrawableRect.centerY(),
                if (isQr!!) mDrawableRadius + 8.dp2Px else mDrawableRadius,   //resources.getDimensionPixelSize(R.dimen.size_8)
                mBitmapPaint
            )
        }
        if (mBorderWidth > 0) {
    ...
    private fun calculateBounds(): RectF {
        ...
        val spacing = 16.dp2Px //resources.getDimensionPixelSize(R.dimen.size_16)

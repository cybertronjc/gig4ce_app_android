...
class ChatPageFragment : Fragment(),PopupMenu.OnMenuItemClickListener /*OnChatMessageClickListener*/ {
...
//            if (fromClientActivation) checkForContact(mobileNumber, username) else username
    ...
    private fun manageMenu(view: View) {
        ...
        popUp.menu.findItem(R.id.action_block).title =
            if (chatFooter.isVisible) "Unblock" else "Block"
        popUp.show()
    }
...
//                    )
//                } else {
//                    if (messageOld.toMessage().attachmentPath != null) {
//                        viewModel.downloadAndSaveFile(
//                            appDirectoryFileRef,
//                            position,
//                            messageOld.toMessage()
//                        )
//                    } else {
//
//                    }
...
//                    pickDocument()
                ...
                Toast.makeText(requireContext(),"Please grant storage permission, to pick files",Toast.LENGTH_SHORT).show()
        }
    }

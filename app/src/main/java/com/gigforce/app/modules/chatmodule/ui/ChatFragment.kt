...
//        layoutManager.reverseLayout = false
    ...
    private fun initIntent() {
        ...
        tv_nameValueInChat.text = if (fromClientActivation) checkForContact(mobileNumber, username) else username
    }
    ...
    override fun onRequestPermissionsResult(
        ...
        if (requestCode == PermissionUtils.reqCodePerm && PermissionUtils.permissionsGrantedCheck(
            ...
            tv_nameValueInChat.text = checkForContact(mobileNumber, username)
        }
    ...
    fun checkForContact(number: String?, name: String): String {
        var nameFromDB = name
        return if (number != null && number.length >= 10 && PermissionUtils.checkForPermissionFragment(this, PermissionUtils.reqCodePerm, Manifest.permission.READ_CONTACTS)) {
            val lookupUri = Uri.withAppendedPath(ContactsContract.PhoneLookup.CONTENT_FILTER_URI, Uri.encode(number))
            val mPhoneNumberProjection = arrayOf<String>(ContactsContract.PhoneLookup._ID, ContactsContract.PhoneLookup.NUMBER, ContactsContract.PhoneLookup.DISPLAY_NAME)
            val cur: Cursor? = requireActivity().contentResolver.query(lookupUri, mPhoneNumberProjection, null, null, null)
            cur.use { cur ->
                if (cur?.moveToFirst() == true) {
                    nameFromDB = cur.getString(cur.getColumnIndex(ContactsContract.Contacts.DISPLAY_NAME_PRIMARY))
